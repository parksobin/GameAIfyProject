...
public class VirusSet : MonoBehaviour
{
    ...
    public int pointsPerSide = 5;         // 꼭짓점 포함 한 면의 점 개수(=5면 16개가 됨)
    public float diamondRotationDeg = 0f; // 마름모 자체 회전(옵션)

    [Header("Move")]
    public float moveTime = 0.6f;         // (0,0,0)→타겟 이동 시간
    ...
    // 각 변을 등분해서 16개 좌표 생성 (면당 5개, 꼭짓점 중복 제외)
    List<Vector3> BuildDiamondTargets(float r, int perSide, float rotDeg)
    {
        ...
        // 각 변을 perSide-1 등분 간격으로 샘플하되,
        // 다음 변의 시작 꼭짓점은 중복이므로 제외 → 4 * (perSide-1)개
