...
public class VirusSet : MonoBehaviour
{
    ...
    {
        Vector3 center = transform.position; // 기준점 고정
        SpawnDiamond(center);
    }
    ...
    IEnumerator MoveTo(Transform tr, Vector3 target, float duration)
    {
        ...
        if (duration <= 0f)
        {
            if (tr) tr.position = target; // 마지막에도 널 가드
            yield break;
        }
        ...
        while (t < duration)
        {
            if (!tr) yield break; // 파괴/비활성 등으로 참조 끊기면 종료
        ...
        if (tr) tr.position = target; // 루프 후 최종 스냅 시에도 체크
    }

    // 이 스크립트가 비활성/파괴될 때 코루틴 정리(안전)
